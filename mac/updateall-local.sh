...
#if you want toactually read the agreement, instead use
...
sudo -H /usr/local/bin/python3 -m pip list --outdated  | awk '{printf "%s",$1;printf "==";print $3}' > temp.txt; sed '1d' temp.txt > temp2.txt;sed '1d' temp2.txt > requirements.txt
sudo -H /usr/local/bin/python3 -m pip install --upgrade -r requirements.txt
rm temp.txt
rm temp2.txt
rm requirements.txt
